...
void UComboGraphAbilityTask_StartGraph::EndGraph()
{
	...
	CG_RUNTIME_LOG(
		Log,
		TEXT("UComboGraphAbilityTask_StartGraph ... Ending graph. CurrentNode: %s (%s)"),
		*GetNameSafe(CurrentNode),
		CurrentNode ? *CurrentNode->GetNodeTitle().ToString() : TEXT("NONE")
	)
...
void UComboGraphAbilityTask_StartGraph::EndGraphWithWaitNetSync()
{
	using namespace UE::ComboGraph;
	const TOptional<EComboGraphAbilityTaskNetSyncType> SyncTypeValue = GetSyncTypeConfigValue(EComboGraphNetworkSyncTime::OnGraphEnd);

	if (!SyncTypeValue.IsSet())
	{
		CG_RUNTIME_WLOG(
			Verbose,
			TEXT("Sync type for %s is set to None, bypassing the wait net sync task and calling EndGraph immediately"),
			*UEnum::GetValueAsString(EComboGraphNetworkSyncTime::OnGraphEnd)
		);
		// Set to None, bypass wait net sync task
		EndGraph();
		return;
	}

	const EComboGraphAbilityTaskNetSyncType SyncType = SyncTypeValue.GetValue();

	CG_RUNTIME_WLOG(
		Verbose,
		TEXT("Sync type for %s is set to %s, setup wait net sync task with %s to invoke EndGraph"),
		*UEnum::GetValueAsString(EComboGraphNetworkSyncTime::OnGraphEnd),
		*UEnum::GetValueAsString(SyncType),
		*UEnum::GetValueAsString(SyncType)
	);
	
	UComboGraphAbilityTask_NetworkSyncPoint* Task = UComboGraphAbilityTask_NetworkSyncPoint::WaitNetSync(Ability, SyncType);
	// The legacy behavior before 1.6.0 was set to None (there was no sync point task used here)
	//
	// The default "None" value matches the previous behavior, but we added this configuration to allow user to tweak it,
	// allowing usage of a network sync task before ending the graph.
	Task->OnSync.AddDynamic(this, &UComboGraphAbilityTask_StartGraph::EndGraph);
	Task->ReadyForActivation();
}
...
void UComboGraphAbilityTask_StartGraph::HandleEventReceived(const FGameplayTag EventTag, const FGameplayEventData EventData)
{
	...
	else
	{
		...
		if (Ability)
		{
			...
			// Following PR #57 and introduction of possibility to register hits on clients (so possibility to send
			// event on server and client).
			//
			// Gate application of cue containers and gameplay cues to only server, not doing so might trigger a
			// crash / ensure error in PackageMapClient.cpp and mismatch between DriverPIEInstanceID and
			// ObjectPIEInstanceID.
			// This happens when a project is using ComboGraph custom GameplayEffectContext and indicates we need a
			// deeper investigation and understanding on how to better solve this.
			//
			// For now, ensure we don't run into this assertion error by making sure we only apply cues on server as
			// before, regardless of collision config (e.g., registering hits and sending events for effect / cue
			// containers on clients).
			//
			// Another related issue was concerning for effect containers and application, but should have been dealt
			// with a WaitNetSync only server wait task to refresh prediction keys before invoking callbacks where we
			// apply containers.
			// Sadly, this didn't suffice for cue containers and custom effect context.
...
{
	CG_RUNTIME_WLOG(Verbose, TEXT("HandleTriggerTransitionEvent ..."))
	HandleComboTransition();
}
...
void UComboGraphAbilityTask_StartGraph::OnServerSyncAdvanceNextNode()
{
	...
	{
		CG_RUNTIME_WLOG(Warning, TEXT("Advance Next node failed: %s"), *FailReason)
		EndGraphWithWaitNetSync();
	}
}
...
void UComboGraphAbilityTask_StartGraph::HandleComboBeginEvent()
{
	...
	CG_RUNTIME_WLOG(Log, TEXT("Open Combo Window: %s"), bComboWindowOpened ? TEXT("true") : TEXT("false"))
}

void UComboGraphAbilityTask_StartGraph::HandleComboEndEvent()
{
	bComboWindowOpened = false;
	CG_RUNTIME_WLOG(Log, TEXT("Close Combo Window: %s (Queued node: %s)"), *GetBoolText(bComboWindowOpened), *GetNameSafe(QueuedNode))
...
{
	using namespace UE::ComboGraph;
	const TOptional<EComboGraphAbilityTaskNetSyncType> SyncTypeValue = GetSyncTypeConfigValue(EComboGraphNetworkSyncTime::OnComboTransition);
	...
	if (!SyncTypeValue.IsSet())
	{
		CG_RUNTIME_WLOG(
			Verbose,
			TEXT("Sync type setting for %s is set to None, bypassing the wait net sync task and calling OnServerSyncAdvanceNextNode immediately"),
			*UEnum::GetValueAsString(EComboGraphNetworkSyncTime::OnComboTransition)
		);
		// Set to None, bypass wait net sync task
	...
	CG_RUNTIME_WLOG(
		Verbose,
		TEXT("Sync type for %s is set to %s, setup wait net sync task with %s to invoke OnServerSyncAdvanceNextNode"),
		*UEnum::GetValueAsString(EComboGraphNetworkSyncTime::OnComboTransition),
		*UEnum::GetValueAsString(SyncType),
		*UEnum::GetValueAsString(SyncType)
	);
	...
	UComboGraphAbilityTask_NetworkSyncPoint* Task = UComboGraphAbilityTask_NetworkSyncPoint::WaitNetSync(Ability, SyncType);
	// The legacy behavior before 1.6.0 was set to OnlyServerWait
