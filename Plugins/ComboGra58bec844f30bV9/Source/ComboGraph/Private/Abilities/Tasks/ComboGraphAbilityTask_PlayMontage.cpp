...
void UComboGraphAbilityTask_PlayMontage::OnGameplayEvent(const FGameplayTag EventTag, const FGameplayEventData* Payload) const
{
	...
	const TOptional<EComboGraphAbilityTaskNetSyncType> SyncTypeValue = GetSyncTypeConfigValue(EComboGraphNetworkSyncTime::OnEventReceived);

	if (!SyncTypeValue.IsSet())
	{
		CG_RUNTIME_WLOG(
			Verbose,
			TEXT("Sync type setting for %s is set to None, bypassing the wait net sync task and calling OnServerSyncEventReceived immediately"),
			*UEnum::GetValueAsString(EComboGraphNetworkSyncTime::OnEventReceived)
		);
		// Set to None, bypass wait net sync task
		OnServerSyncEventReceived(EventTag, TempData);
		return;
	}
	...
	CG_RUNTIME_WLOG(
		Verbose,
		TEXT("Sync type setting for %s is set to %s, setup wait net sync task with %s to invoke OnServerSyncEventReceived"),
		*UEnum::GetValueAsString(EComboGraphNetworkSyncTime::OnEventReceived),
	...
	Task->OnSyncDelegate.AddUObject(this, &UComboGraphAbilityTask_PlayMontage::OnServerSyncEventReceived, EventTag, TempData);
	Task->ReadyForActivation();
}

void UComboGraphAbilityTask_PlayMontage::OnServerSyncEventReceived(FGameplayTag InEventTag, FGameplayEventData InPayload) const
{
	if (ShouldBroadcastAbilityTaskDelegates())
	{
		EventReceived.Broadcast(InEventTag, InPayload);
	}
}
