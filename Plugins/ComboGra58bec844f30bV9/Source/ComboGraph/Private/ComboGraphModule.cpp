...
void FComboGraphModule::StartupModule()
{
	...
	FCoreDelegates::OnPostEngineInit.AddStatic(&FComboGraphModule::OnPostEngineInit);
}
...
void FComboGraphModule::ShutdownModule()
{
	...
	// Remove delegates (even though we now bind static, so below is not strictly necessary)
...
void FComboGraphModule::SetupSettings()
{
	...
	if (ISettingsModule* SettingsModule = FModuleManager::GetModulePtr<ISettingsModule>("Settings"))
	{
		...
		{
			SettingsSection->OnModified().BindStatic(&FComboGraphModule::HandleSettingsSaved);
		}
	}
#endif
}
...
void FComboGraphModule::ShutdownSettings()
{
	...
	if (ISettingsModule* SettingsModule = FModuleManager::GetModulePtr<ISettingsModule>("Settings"))
	{
		SettingsModule->UnregisterSettings(TEXT("Project"), TEXT("Plugins"), TEXT("ComboGraph"));
	}
#endif
}
...
void FComboGraphModule::EnsureGlobalData()
{
	// Note: As of now, since 5.3, below code is not necessary anymore
...
bool FComboGraphModule::HandleSettingsSaved()
{
	...
	if (!bValid)
	{
		...
		FNotificationInfo Info(LOCTEXT(
			"SettingsHasNoComboWindow",
			"Notifies State Auto Setup must include Combo Window.\n\nSettings has been updated to include it with default value."
		));
		// Info.Image = nullptr;
