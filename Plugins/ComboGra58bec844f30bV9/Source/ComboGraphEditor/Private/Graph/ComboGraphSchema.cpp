...
UEdGraphNode* FComboGraphSchemaAction_AddComment::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
...
UEdGraphNode* FComboGraphSchemaAction_NewNode::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, const bool bSelectNewNode)
{
...
UEdGraphNode* FComboGraphSchemaAction_NewEdge::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
	UEdGraphNode* ResultNode = nullptr;
...
UEdGraphNode* FComboGraphSchemaAction_NewNodeConduit::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
	UEdGraphNode* ResultNode = nullptr;
...
UEdGraphNode* FComboGraphSchemaAction_AutoArrangeVertical::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
...
UEdGraphNode* FComboGraphSchemaAction_AutoArrangeHorizontal::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const FVector2f& Location, bool bSelectNewNode)
{
	UComboGraphEdGraph* Graph = Cast<UComboGraphEdGraph>(ParentGraph);
	if (Graph)
	{
...
void UComboGraphSchema::DroppedAssetsOnGraph(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraph* Graph) const
{
...
void UComboGraphSchema::DroppedAssetsOnNode(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphNode* Node) const
{
...
void UComboGraphSchema::DroppedAssetsOnPin(const TArray<FAssetData>& Assets, const FVector2f& GraphPosition, UEdGraphPin* Pin) const
{
	UAnimationAsset* Asset = FAssetData::GetFirstAsset<UAnimationAsset>(Assets);

	if (Asset && Pin)
	{
		const FVector2f FixedOffset = GetFixedOffsetFromPin(Pin);
...
void UComboGraphSchema::SpawnNodeFromAsset(UAnimationAsset* Asset, const FVector2f& GraphPosition, UEdGraph* Graph, UEdGraphPin* PinIfAvailable)
{
	check(Graph);
...
{
	const FVector2f InitPos((OwningNodeA->NodePosX + OwningNodeB->NodePosX) / 2, (OwningNodeA->NodePosY + OwningNodeB->NodePosY) / 2);
...
FVector2f UComboGraphSchema::GetFixedOffsetFromPin(const UEdGraphPin* InHoverPin)
{
	...
	FVector2f FixedOffset(450.0f, 0.f);
	FVector2f DesiredSize(0.f, 0.f);

	if (const UComboGraphEdNode* EdGraphNode = Cast<UComboGraphEdNode>(InHoverPin->GetOwningNode()))
	{
		if (EdGraphNode->SlateNode)
		{
			const FVector2D ComputeDesiredSize = EdGraphNode->SlateNode->ComputeDesiredSize(FSlateApplication::Get().GetApplicationScale());
			DesiredSize = FVector2f(ComputeDesiredSize.X, ComputeDesiredSize.Y);
		}
	}

	if (InHoverPin->PinName == UComboGraphPinNames::PinName_InLeft)
	{
		FixedOffset = FVector2f(-(DesiredSize.X * 2 + 100.f), -DesiredSize.Y);
	}
	else if (InHoverPin->PinName == UComboGraphPinNames::PinName_InTop)
	{
		FixedOffset = FVector2f(-DesiredSize.X, -(DesiredSize.Y * 2 + 100.f));
	}
	else if (InHoverPin->PinName == UComboGraphPinNames::PinName_OutRight)
	{
		FixedOffset = FVector2f(100.f, -DesiredSize.Y);
	}
	else if (InHoverPin->PinName == UComboGraphPinNames::PinName_OutBottom)
	{
		FixedOffset = FVector2f(-DesiredSize.X, 100.f);
	}
