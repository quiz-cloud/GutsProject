		...
		if (Pin)
		{
			...
			for (int32 Idx = 0; Idx < TempLinkedTo.Num(); Idx++)
			{
				...
				if (GraphNode && BBoxTree.Children.Num() > 0)
				{
					...
					GraphNode->DEPRECATED_NodeWidget.Pin()->MoveTo(FVector2f(BBoxTree.Children[BBoxIndex].SubGraphBBox.X / 2 - GraphNode->DEPRECATED_NodeWidget.Pin()->GetDesiredSize().X / 2 + PosX, PosY), NodeFilter);
		...
		if (Pin)
		{
			...
			for (int32 Idx = 0; Idx < TempLinkedTo.Num(); Idx++)
			{
				...
				if (GraphNode && BBoxTree.Children.Num() > 0)
				{
					...
					GraphNode->DEPRECATED_NodeWidget.Pin()->MoveTo(FVector2f(PosX, BBoxTree.Children[BBoxIndex].SubGraphBBox.Y / 2 - GraphNode->DEPRECATED_NodeWidget.Pin()->GetDesiredSize().Y / 2 + PosY), NodeFilter);
...
void UComboGraphEdGraph::AutoArrange(const bool bVertical)
{
	...
	if (bVertical)
	{
		...
		RootNode->DEPRECATED_NodeWidget.Pin()->MoveTo(FVector2f(NewRootPosX, 0), NodeFilter);
	}
	else
	{
		...
		RootNode->DEPRECATED_NodeWidget.Pin()->MoveTo(FVector2f(0, NewRootPosY), NodeFilter);
	}
